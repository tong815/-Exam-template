...
示例：`"stem": "Find the zeros of \\(x^2-9x+20\\)."`  
货币保持纯文本：`"stem": "Each item costs $10 and revenue is \\(R=10n\\)."`  
旧数据中的 `$...$` 在打开时会自动转为 `\\( ... \\)`；`x^2`、`2^(x+1)` 等仍有无 `$`/`\\(` 时的自动迁移层。
